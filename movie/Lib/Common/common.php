...
function getpage($currentPage,$totalPages,$halfPer=3,$url,$pagego){
    $linkPage .= ( $currentPage > 1 )
        ? '<a href="'.str_replace('{!page!}',1,$url).'" class="prev disabled" data="p-0">首页</a>&nbsp;<a href="'.str_replace('{!page!}',($currentPage-1),$url).'" class="prev disabled" data="p-'.($currentPage-1).'">上一页</a>&nbsp;' 
        : '<span class="prev disabled">首页</span>&nbsp;<span class="prev disabled" data="p-'.($currentPage-1).'">上一页</span>&nbsp;';
...
function ff_mcat_url($str,$cid)
{
	...
	$html = '';
	foreach($mcids as $v){
		$url = UU('Home-vod/type', array('id'=>$cid, 'mcid'=>$v),false,true);
...
function ff_mysql_vod($tag){
	...
	if ($tag['exp']) {
		$where["FIND_IN_SET({$tag['exp']},vod_exp)"] = array("gt","0");
	}
	if ($tag['prty']) {
	...
	if ($tag['letter']) {
		...
		if ($tag['weekday']) {
		$where['vod_weekday'] = array('like','%'.$tag['weekday'].'%');
	}
	if($tag['isfiml']){
	...
	if($tag['page']){
		...
		$pageurl = C('jumpurl');
		$pages = '共'.$count.'部&nbsp;'.$currentpage.'/'.$totalpages.'&nbsp;'.getpage($currentpage,$totalpages,C('home_pagenum'),$pageurl,'pagego(\''.$pageurl.'\','.$totalpages.')');
		$pagestop = ''.$currentpage.'/'.$totalpages.'&nbsp;'.getpagetop($currentpage,$totalpages,C('home_pagenum'),$pageurl,'pagego(\''.$pageurl.'\','.$totalpages.')');	
...
function home_hot_play() {
	...
	$list = $model->field('vod_id,vod_name,vod_filmtime,vod_cid,vod_jumpurl')->where($where)->order($order)->limit(8)->select();
	if($list) {
		foreach ($list as $key => $val) {
 ...
 */
 function home_after_playtv() {
	...
	$list = $model->field('vod_id,vod_filmtime,vod_name,vod_cid,vod_jumpurl')->where($where)->order($order)->limit(8)->select();
	if($list) {
		foreach ($list as $key => $val) {
			$list[$key]['vod_readurl'] = ff_data_url('vod',$val['vod_id'],$val['vod_cid'],$val['vod_name'],1,$val['vod_jumpurl']);
		}
